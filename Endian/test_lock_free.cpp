...
// 当前活跃（pointer 非空）的 hazard 数 —— 用来观察域的增长/复用
static size_t live_hazards() {
    return hp::hp_owner::hazard_domain.protected_ptrs().size();
}
// 强制回收（无视批量阈值）：用于断言“此刻应已无残留”
static void force_drain() {
    for (int i = 0; i < 3; ++i) lock_free::delete_nodes_with_no_hazards(true);
}
...
static void test_stack_hazard_reclaim() {
    ...
    force_drain();
    TEST("stack: 回收链表最终清空", lock_free::nodes_to_reclaim.load() == nullptr);
...
static void test_unbounded_deep_nesting() {
    const long base = Guarded::alive.load();
    constexpr int N = 300;             // 旧版 128 上限会在此处抛异常；新版应顺利增长
    {
...
static void test_global_balance() {
    ...
    TEST("收尾: 回收链表为空", lock_free::nodes_to_reclaim.load() == nullptr);
